...
from .models import( Genre, Movie, MovieCast, MovieImage, MovieImageExtra, MovieStaff, Rating, Role, TitleMovie, Type,)
from django.forms.widgets import DateInput
from core.utils.validators import validate_file_extension, validate_url, ALLOWED_IMAGE_EXTENSIONS
from django.utils.translation import gettext_lazy as _
from apps.common.models import Company, ImageSize, Person, Language
...
########################################################################################################    Formulario para Movie
class MovieForm(forms.ModelForm):
    class Meta:
        model = Movie
        fields = [
            'title', 'title_secundary', 'release_year', 'duration_minutes', 'synopsis',
            'movie_types', 'movie_rating', 'genres', 'producers', 'distributors', 'is_active'
        ]
    ...
    title = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={
                'class': 'form-control',
                'placeholder': _('Título original de la película'),
                'autocomplete': 'off',
                'type': 'text',
                'id': 'title'
            }
        ...
        error_messages={
            'required': _('Debe ingresar el título original de la película.'),
    ...
    title_secundary = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={
                'class': 'form-control',
                'placeholder': _('Título secundario de la película'),
                'autocomplete': 'off',
                'type': 'text',
                'id': 'title_secundary'
            }
    ...
    synopsis = forms.CharField(
        ...
        widget=forms.Textarea(
            attrs={
                'class': 'form-control',
                'placeholder': _('Sinopsis de la película'),
                'rows': 4,
                'id': 'synopsis'
            }
        ),
    ...
    movie_types = forms.ModelChoiceField(
        queryset=None,
        required=True,
        empty_label=_('Seleccione un tipo de película'),
        label=_('Tipo Película'),
        widget=forms.Select(
            attrs={
                'class': 'form-select',
                'id': 'movie_types',
        ...
        error_messages={
            'required': _('Debe seleccionar un tipo de película.'),
        },
    )

    movie_rating = forms.ModelChoiceField(
        queryset=None,
        required=True,
        empty_label=_('Seleccione una clasificación'),
        label=_('Clasificación Película'),
        widget=forms.Select(
            attrs={
                'class': 'form-select',
                'id': 'movie_rating',
    ...
    is_active = forms.BooleanField(
        ...
        widget=forms.CheckboxInput(
            attrs={
                'class': 'form-check-input',
                'id': 'is_active',
                'title': _('Marcar para activar o desmarcar para desactivar la película.'),
            }
        ),
    )
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['movie_types'].queryset = Type.objects.filter(is_active=True)
        self.fields['movie_rating'].queryset = Rating.objects.filter(is_active=True)
    ...
    def clean(self):
        ...
        qs = Movie.objects.exclude(pk=self.instance.pk if self.instance else None)

        for movie in qs:
            # Validar que no exista película con misma combinación título, título secundario y año
            if (normalize_text(movie.title) == normalized_title and normalize_text(movie.title_secundary or '') == normalized_title_secundary and movie.release_year == release_year):
                raise forms.ValidationError(_('Ya existe una película con el mismo título, título secundario y año de lanzamiento.'))

        return cleaned_data

########################################################################################################    Formulario para MovieCast
class MovieCastForm(forms.ModelForm):
    class Meta:
        model = MovieCast
        fields = ['movie', 'actor', 'role', 'character_name', 'is_active']
    ...
    movie = forms.ModelChoiceField(
        ...
        error_messages={
            'required': _('Debe seleccionar una película.'),
        }
    )
    ...
    def clean(self):
        ...
        if not all([movie, actor, role, character_name]):
        ...
        existing = MovieCast.objects.exclude(pk=self.instance.pk).filter(
            movie=movie,
            actor=actor,
            role=role,
        )
        ...
        for cast in existing:
            if normalize_text(cast.character_name) == normalized_character:
                raise forms.ValidationError( _('Ya existe un registro de este actor interpretando a este personaje en esa película con ese rol.'))

        return cleaned_data

########################################################################################################    Formulario para MovieImage
class MovieImageForm(forms.ModelForm):
    class Meta:
        model = MovieImage
        fields = ['movie', 'size_image', 'image', 'image_url', 'is_active']

    movie = forms.ModelChoiceField(
        queryset=None,
        required=True,
        label=_('Película'),
        empty_label=_('Seleccione una película'),
        widget=forms.Select(
            attrs={
                'class': 'form-select',
                'id': 'movie'
    ...
    is_active = forms.BooleanField(
        ...
        widget=forms.CheckboxInput(
            attrs={
                'class': 'form-check-input',
                'id': 'is_active',
                'title': _('Marcar para activar o desmarcar para desactivar la imagen de la película.'),
            }
        )
    )
...
########################################################################################################    Formulario para MovieImageExtra
class MovieImageExtraForm(forms.ModelForm):
    class Meta:
        model = MovieImageExtra
        fields = ['movie', 'image', 'is_active']
    ...
    is_active = forms.BooleanField(
        ...
        widget=forms.CheckboxInput(
            attrs={
                'class': 'form-check-input',
                'id': 'is_active',
                'title': _('Marcar para activar o desmarcar para desactivar la imagen adicional de la película.'),
            }
        )
    )
...
########################################################################################################    Formulario para MovieStaff
class MovieStaffForm(forms.ModelForm):
    class Meta:
        model = MovieStaff
        fields = ['movie', 'person', 'role', 'is_active']

    movie = forms.ModelChoiceField(
        queryset=Movie.objects.filter(is_active=True),
        label=_('Película'),
        widget=forms.Select(attrs={'class': 'form-select', 'id': 'movie'}),
        required=True,
    )
    ...
    def clean(self):
        ...
        if not all([movie, person, role]):
            return cleaned_data

        qs = MovieStaff.objects.exclude(pk=self.instance.pk).filter( movie=movie, person=person, role=role,)

        if qs.exists():
            raise forms.ValidationError( _('Ya existe un registro con esa combinación de película, persona y rol.'))
...
########################################################################################################    Formulario para TitleMovie
class TitleMovieForm(forms.ModelForm):
    class Meta:
        model = TitleMovie
        fields = ['movie', 'title_lang', 'title', 'is_active']
    ...
    def clean(self):
        ...
        if movie and title_lang and title:
            title = title.strip()

            exists = TitleMovie.objects.exclude(pk=self.instance.pk).filter(
                movie=movie,
            ...
            if exists.exists():
                raise forms.ValidationError(
                    _('Ya existe este título para la película en ese idioma.')
                )
